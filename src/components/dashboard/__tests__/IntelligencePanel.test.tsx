...
const createHealthCheckResult = (score: number, status: 'healthy' | 'needs_attention' | 'critical') => ({
  overall_score: score,
  metrics: {
...
describe('IntelligencePanel', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetHealthCheck.mockResolvedValue(createHealthCheckResult(85, 'healthy'));
  ...
  describe('Full Panel Mode', () => {
    ...
    it('shows needs_attention status for medium scores', async () => {
      mockGetHealthCheck.mockResolvedValue(createHealthCheckResult(55, 'needs_attention'));
    ...
    it('shows no duplicates message when empty', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Your memory bank is well organized')).toBeInTheDocument();
      });
    });
    ...
    it('shows duplicate pairs when found', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Found 1 potential duplicate pairs')).toBeInTheDocument();
  ...
  describe('Color Coding', () => {
    ...
    it('shows yellow color for scores between 60-79', async () => {
      mockGetHealthCheck.mockResolvedValue(createHealthCheckResult(65, 'needs_attention'));
    ...
    it('shows red color for scores < 60', async () => {
      mockGetHealthCheck.mockResolvedValue(createHealthCheckResult(45, 'critical'));
